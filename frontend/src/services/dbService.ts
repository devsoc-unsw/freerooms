...
const api = {
  ...
  getRoomsInBuilding: handleError(
    async (buildingName: string, date: string) => {
  ...
  getRoomBookingsInTimeRange: handleError(
    async (roomName: string, startDate: string, endDate: string) => {
      // Ainsworth room 101 week 1.
      const res = await axios.get(
        baseURL + "buildings/" + "K-J17" + "/" + "101" + "/" + "1"
      );
      return res.data;
    }
  ),
...
export default class DbService {
  ...
  async getRoomsInBuilding(buildingName: string, hour: string) {
    ...
    for (const room of data) {
      ...
      curResult["available"] = this.checkAvailable(
  ...
  checkAvailable(bookings: any[], hour: string) {
    for (const timePair of bookings) {
      ...
      if (+startHour <= +hour && +hour < +endHour) return false;
    }
    return true;
  }
