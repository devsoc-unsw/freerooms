...
async function check(directory) {
    for (const entry of await readdir(directory, { withFileTypes: true })) {
        ...
        } else if (entry.isFile() && extensions.test(entry.name) && !/\.test\.[jt]sx?$/.test(entry.name)) {
            const lines = (await readFile(path, "utf-8")).split(/\r?\n/);

            for (let i = 0; i < lines.length; i++) {
                if (rawColour.test(lines[i])) {
                    console.error(`${relative(root, path)}:${i + 1}: Use semantic them colour instead of raw colour.`);
                    failures++;
                }
...
if (failures) {
    console.error(`Found ${failures} lines containing raw colours outside the theme.`);
    process.exitCode = 1;
} else {
