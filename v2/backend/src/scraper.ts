...
async function scrapeAllBuildings() {
    ...
    for await (const buildingName of buildingsToScrape) { buildingPromises.push(scrapeBuilding(buildingName)); }
...
const scrapeBuilding = async (buildingName: string): Promise<BuildingData> => {
    ...
    const buildingImageUrl = buildingPage?.getElementById('block-page-sidebar')?.querySelector('img[typeof="foaf:Image"]')?.getAttribute("src") ?? "";
    const buildingId = buildingPage?.querySelector(".field--name-field-building-id")?.querySelector(".field-item")?.innerHTML ?? "";
...
const scrapeRoom = async (roomName: string): Promise<RoomData> => {
    ...
    const roomCapacity = roomPage?.querySelector(".field--name-field-room-capacity")?.querySelector(".field-item")?.innerHTML;
    const usageOptions = parseRoomUsageOptions(roomPage?.querySelector(".field--name-field-room-usage")?.querySelector(".field-item")?.innerHTML);
...
async function* getAllScrapeableEntitiesFromCards(
    ...
    while (true) {
        ...
        for (const card of cards) {
            const entity = card.querySelector(".teaser-link")?.querySelector("a")?.getAttribute("href");
            if (entity) { 
                yield entity; 
            }            
        }
