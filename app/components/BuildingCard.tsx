...
const BuildingCard: React.FC<BuildingCardProps> = ({
  ...
  const imgurl = {
    uri: `https://freerooms.staging.csesoc.unsw.edu.au/_next/image?url=%2Fassets%2Fbuilding_photos%2F${id}.webp&w=1920&q=75`,
  };
      ...
      <View style={styles.cardOverlay}>
        <Pressable
          ...
          style={({ pressed }) => [
            ...
            pressed
              ? { backgroundColor: "rgba(0, 0, 0, 0.25)", borderRadius: 15 }
              : {},
          ]}
        >
          <View style={styles.textWrapper}>
            <Text style={styles.card_heading}>{`${name}`}</Text>
            <Text style={styles.card_subheading}>{`(${id})`}</Text>
          </View>
